...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addShortcode("currentYear", function (value, separator = ',') {
  ...
  eleventyConfig.addShortcode("mainMenuItem", function (label, location, fields) {
    if (location && location.slice(-1) !== '/') location += '/';
    const dataAttrs = [];
    for (field in fields) {
      dataAttrs.push(`data-${field}="${fields[field]}"`)
    }
    return `
            <a class="grid items-center justify-center bg-blue-fbc w-full h-48 rounded" href="${location}" ${dataAttrs.join(' ')}>
              <span class="font-headline font-bold text-2xl text-white">${label}</span>
            </a>
        `;
  });

  eleventyConfig.addShortcode("videoButton", function (ytId, h, p, textPosition = 'below', fields) {
    const dataAttrs = [];
    for (field in fields) {
      dataAttrs.push(`data-${field}="${fields[field]}"`)
    }
  ...
  eleventyConfig.addShortcode('slimAd', function (adText) {
    return `
  ...
  eleventyConfig.addShortcode('fullWidthQuote', function (quoteText) {
    return `
  ...
  eleventyConfig.addShortcode("contentBlock", function (contentBlock) {
    return `
                    <section id="${contentBlock.fields.sectionLink}">
                        <div class="wrapper">
                            <div class="inner">
                                <h3 class="major">${contentBlock.fields.sectionTitle
      }</h3>
  ...
  eleventyConfig.addShortcode("cardBlock", async function (cardBlock) {
        ...
        return (cards = client.getEntry(sys.id).then((card) => {
          return `<article>
            ...
            card.fields.image
                                    ...
                                    <h3 class="major">${card.fields.sectionTitle
            }</h3>
    ...
    return `
                    <section id="${cardBlock.fields.sectionLink
      }" class="wrapper alt style1">
                        <div class="inner">
                            <h2 class="major">${cardBlock.fields.sectionTitle
      }</h2>
    ...
    if (featuretteBlock.fields.imageLocation) {
      return `
          ...
          featuretteBlock.fields.image
                                ...
                                <div class="content">
                                    <h2 class="major">${featuretteBlock.fields.sectionTitle
        }</h2>
                                    ${documentToHtmlString(
