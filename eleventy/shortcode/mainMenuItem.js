module.exports = function (fields) {
  let location = fields?.page?.fields?.slug;
    ...
    return `
            <a class="w-full h-48 rounded bg-cover bg-center" href="${location}" ${dataAttrs.join(' ')} style="background-image: url('${bgImage}')">
              <div class="filter bg-blue-fbc w-full h-full grid items-center justify-center bg-opacity-75">
                <span class="font-headline font-bold text-2xl text-white">${fields.label}</span>
              </div>
            </a>
        `;
}